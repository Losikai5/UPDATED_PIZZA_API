...
async def create_order(order: OrderCreate, session: AsyncSession = Depends(get_session), token_data: dict = Depends(bearer_validator)):
    user_id = token_data.get("user")["uid"]
    create_order = await order_service.create_order(order, user_id, session)
    return create_order  

@Orders_router.get("/user/{order_id}", response_model=OrderRead, dependencies=[workers_role])
async def get_order_by_user(order_id: str, session: AsyncSession = Depends(get_session), token_data: dict = Depends(bearer_validator)): 
    get_order_by_id = await order_service.get_order_by_user(order_id, session)
